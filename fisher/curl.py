...
def parse_page_data_low(data_file, goods, doc_sort, wwsy_result):
	for i in range(len(goods)):
		...
		countOfarticle = '0'
		"""
		if len(goods[i]['countOfarticle']) < 1:
			countOfarticle = '0'
		else:
			countOfarticle = goods[i]['countOfarticle']
		"""
		...
		data_doc = str(doc_sort) + '\t' + goods[i]['partnumberVendorId'] + '\t' + '0' + '\t' + doc_score['_DY_price'] + '\t' + '0' + '\t' + '0' + '\t' + '0' + '\t' + countOfarticle + '\n'
...
def parse_page_data(data_file, goods, doc_sort, wwsy_result):
	for i in range(len(goods)):
		...
		data_doc = str(doc_sort) + '\t' + goods[i]['partnumberVendorId'] + '\t' + doc_score['totalScore'] + '\t' + doc_score['_DY_price'] + '\t' + goodsQualityScore + '\t' + goods[i]['threeGroupId'] + '\t' + totalCount + '\t' + countOfarticle + '\n'
		data_file.write(data_doc)
	return doc_sort
...
if __name__ == '__main__':
	...
	for i in range(process_count):
		p = Process(target=main, args=(query, data_path, fail_name, process_count, i, lock))
		process.append(p)
		# p.setDaemon(True)
		p.start()
	for p in process:
