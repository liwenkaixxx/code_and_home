    ...
    IntAA& J, int K, double& result)
{
    ...
    for (i=0; i<B_CNT; i++)
    {
        ...
        {
            printf("%0.3lf  ", range[i][j]);
        }
        printf("mean%d: %0.3lf", i, mean[i]);
        printf("\n");
    }

    printf("mean: %0.3lf %0.3lf\n", mean[1]/mean[0], mean[2]/mean[1]);
    if (mean[1]/mean[0] < 1/threshold)
    {
        if (mean[2]/mean[1] < 1/threshold)
        {
            printf("price range: %0.3lf\n", range[2][0]);
        }
        else
